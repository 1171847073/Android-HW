...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        canvas.drawCircle(centerX,centerY,radius / 50, numberPaint);
        ...
        int miniteDegree = (minute - 15) * 6;
        endX = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(miniteDegree)));
        endY = (float) (centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(miniteDegree)));
        canvas.drawLine(centerX, centerY, endX, endY, needlePaint);

        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        int secondDegree = (second - 15) * 6;
    ...
    private void drawTimeNumbers(Canvas canvas) {
        ...
        for (int i = 1; i <= 12; i++) {
            int Degree = 30 * i - 90;
            float endX = (float) (centerX + radius * 0.8 * Math.cos(Math.toRadians(Degree))) - 45;
            float endY = (float) (centerY + radius * 0.8 * Math.sin(Math.toRadians(Degree))) + 35;
            if(i < 10) canvas.drawText("0" + i, endX, endY, numberPaint);
            else canvas.drawText(i + "", endX, endY, numberPaint);
        }
    }
